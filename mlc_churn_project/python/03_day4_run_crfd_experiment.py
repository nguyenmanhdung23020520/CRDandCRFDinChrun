"""
Ngay 4 - Thi nghiem CRFD cho Random Forest

Muc tieu:
- Danh gia anh huong cua hai yeu to k va max_depth den F1-score.
...
- Day 4 chi lam CRFD, khong lam lai CRD hay baseline.
"""
...
# 1. Doc du lieu
# ---------------------------------------------------------
print("===== BAT DAU THI NGHIEM CRFD - DAY 4 =====")
...
if not data_path.exists():
    raise FileNotFoundError(
        ...
        "'python python/03_day4_run_crfd_experiment.py' tu thu muc goc cua project mlc_churn_project."
    )
...
# ---------------------------------------------------------
# 5. Chay repeated stratified k-fold cho CRFD
# ---------------------------------------------------------
k_values = [3, 5, 10]
...
for max_depth in max_depth_values:
    max_depth_label = "None" if max_depth is None else str(max_depth)
    print(f"\nDang chay CRFD voi max_depth = {max_depth_label}")

    for k in k_values:
        print(f"  Dang chay k = {k}")
...
# 6. Luu ket qua
# ---------------------------------------------------------
results_path = results_dir / "crfd_results.csv"
results_df.to_csv(results_path, index=False)

rows_by_depth = results_df.groupby("max_depth").size().reset_index(name="n_rows")
rows_by_k = results_df.groupby("k").size().reset_index(name="n_rows")
rows_by_k_depth = (
    results_df.groupby(["k", "max_depth"])
    .size()
    .reset_index(name="n_rows")
    .sort_values(by=["max_depth", "k"])
)
mean_f1_by_k_depth = (
    results_df.groupby(["k", "max_depth"], as_index=False)["f1"]
    .mean()
    .sort_values(by=["max_depth", "k"])
)

print("\n===== TOM TAT KET QUA CRFD =====")
print("So dong ket qua:")
print(len(results_df))

print("\nSo dong theo tung max_depth:")
print(rows_by_depth)

print("\nSo dong theo tung k:")
print(rows_by_k)

print("\nSo dong theo tung to hop k va max_depth:")
print(rows_by_k_depth)

print("\nMean F1 theo tung to hop k va max_depth:")
print(mean_f1_by_k_depth)

print(f"\nDa luu ket qua tai: {results_path}")
print("===== KET THUC THI NGHIEM CRFD - DAY 4 =====")
